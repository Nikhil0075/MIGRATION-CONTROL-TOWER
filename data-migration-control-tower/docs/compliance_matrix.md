...
| Scale evaluation harness | `evaluation/scale_harness.py` (control-plane object-count tiers, now supporting the Deploy & Harden Phase 4-approved 1k/5k/20k tiers alongside the original 100-500 pattern-scale bound) + `evaluation/data_plane_scale_test.py` (real rows/bytes moved) + `evaluation/load_test.py` (concurrent operational load) + `evaluation/estimate_ladder_cost.py` (full-stack cost estimate, required before any large run) | `evaluation/reports/scale_metrics_{tier}.md`, `data_plane_scale_metrics.md`, `load_test_metrics.md` — real measured throughput/latency, never one folded-together number (docs/EVALUATION.md's three-measurement split) | ⚠️ implemented in code and unit-tested (`tests/test_scale_harness.py`, `tests/test_data_plane_scale_test.py`, `tests/test_load_test.py`, `tests/test_estimate_ladder_cost.py`); the actual 1k/5k/20k control-plane runs have not been executed live yet — pending the cost-estimate confirmation checkpoint. **Still never call any of this "a 20,000-object migration"** — the control-plane tier measures metadata object definitions, moves zero rows |
