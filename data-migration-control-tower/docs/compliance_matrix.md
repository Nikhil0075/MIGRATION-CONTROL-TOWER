...
| Agent Identity — zero-trust access control | Per-agent IAM service accounts with genuinely differing role bindings, declared for all 6 agents; deterministic policy engine as the actual enforcement point; typed OIDC-verified HTTP dispatch (`tools/capability_http_server.py`, `tools/capability_dispatch_client.py`) so a `cloud_run`-runtime AgentCard would genuinely run under its own SA rather than the orchestrator's | `infrastructure/gcp_setup.sh`'s per-agent SA creation; `tools/policy_engine.py` denial records naming the acting identity; `infrastructure/terraform/cloud_run.tf`'s per-service `allowed_callers` IAM bindings | ⚠️ partial — the SA-attachment mechanism is real and verified live for `hello-agent`; the typed-dispatch mechanism that would make the other 8 SAs actually exercised at runtime is implemented and unit-tested (Deploy & Harden Phase 2c) but no real AgentCard has been flipped to `runtime.type=cloud_run` yet — every card still resolves via `local` dynamic import inside the orchestrator process |
| Agent Gateway — unified routing and policy enforcement | Two-layer policy enforcement (Deploy & Harden Phase 1a/1b, `docs/GOVERNANCE.md`): a capability-dispatch gate in `tools/registry.py::invoke_capability()` now covers all 7 agents (`tests/test_policy_engine.py::test_every_seeded_agent_capability_passes_the_dispatch_gate`); tool-level `tools/policy_engine.py::authorize()` is fully wired through Discovery, and was already real for Risk/Cutover's own fine-grained checks | `policy_decisions` records on every audited action; S-03/S-09 in the evaluation harness; `docs/adr/0001-two-layer-policy-enforcement.md` | ⚠️ partial, but materially wider than before — the coarse dispatch gate is now universal; tool-level authorization still needs wiring through Lineage, Planner, and Validation (stated precisely in `docs/GOVERNANCE.md`, not claimed as "universal") |
...
| Data-plane / control-plane separation | `tools/migration_executor.py`'s `DataPlaneExecutor` interface; `InMemoryExecutor` (default) plus `CloudRunJobExecutor` (Deploy & Harden Phase 3, opt-in via `DATA_PLANE_EXECUTOR=cloud_run_job`) — a genuinely separately-deployed Cloud Run Job, not a Dataflow pipeline, but a real second implementation with its own container/service account, not just an interface with one substitution behind it | `tests/test_migration_executor.py`, `tests/test_cloud_run_job_executor.py`, `tests/test_orchestrator.py`'s async-flow tests; streaming via `fetchmany()`/adapter `fetch_rows()` batches instead of one `fetchall()` | ⚠️ implemented in code and unit-tested; not yet live-deployed (`infrastructure/terraform`'s `enable_data_plane_job`/`enable_cloud_sql` are both `false` by default) — still not a managed Dataflow pipeline, called out as a Cloud Run Job explicitly rather than implied to be more than it is |
| Scale evaluation harness | `evaluation/scale_harness.py` (control-plane object-count tiers, now supporting the Deploy & Harden Phase 4-approved 1k/5k/20k tiers alongside the original 100-500 pattern-scale bound) + `evaluation/data_plane_scale_test.py` (real rows/bytes moved) + `evaluation/load_test.py` (concurrent operational load) + `evaluation/estimate_ladder_cost.py` (full-stack cost estimate, required before any large run) | `evaluation/reports/scale_metrics_{tier}.md` — real, live-measured numbers for all three approved tiers (1k: 135.0 rec/sec, 6/994 admitted/held; 5k: 152.5 rec/sec, 6/4994; 20k: 126.9 rec/sec, 132,613.5 items/sec wave throughput, 6/19994) — captured after confirming the full-stack cost estimate ($0.000184 for the 20k tier) and never conflated with a data-plane row count | ✅ implemented, unit-tested (`tests/test_scale_harness.py`, `tests/test_data_plane_scale_test.py`, `tests/test_load_test.py`, `tests/test_estimate_ladder_cost.py`), **and live-executed** for the control-plane 1k/5k/20k ladder. `data_plane_scale_test.py`/`load_test.py` themselves remain code-complete and unit-tested but not yet run against a live deployed estate (needs Phase 3's Cloud SQL/data-plane-job flags flipped on first). **Still never call any of this "a 20,000-object migration"** — the control-plane tier measures metadata object definitions, moves zero rows |

## Deploy & Harden Phase 5: public-release hardening

| Item | What's real | Evidence | Status |
|---|---|---|---|
| Report-generation rate limiting | `frontend/report_service.py::consume_report_quota()` — Firestore-transaction daily quota per user (`report_daily_usage` collection, `REPORT_DAILY_LIMIT=20` default), mirrors the assistant's existing `_consume_quota()` pattern; wired into `frontend/api_v1.py::create_report()` after the idempotency short-circuit so retries of an already-queued report never double-count | `tests/test_report_service.py` (3/3: allows-under-limit, refuses-at-limit-with-429, scoped-per-user) | ✅ implemented and unit-tested |
| `/legacy` route gating | `frontend/app.py::_legacy_ui_enabled()` — the route now 404s by default; reachable only with `ENABLE_LEGACY_UI` set truthy | `tests/test_frontend_v1.py::test_legacy_shell_is_404_by_default`, `::test_legacy_shell_is_reachable_when_explicitly_enabled` | ✅ implemented and unit-tested |
| Firestore direct-access security rules | `firestore.rules` — deny-all (`allow read, write: if false` on every path), justified by confirming the frontend client bundle has zero references to the Firebase Firestore client SDK (grep, empty result) — every read/write is server-mediated through the Admin SDK, which bypasses rules entirely, so the app never needs client-SDK access at all | `tests/test_firestore_rules.py` (5/5, static content assertions — no Firebase emulator available in this dev environment to run the real rules-testing SDK against) | ⚠️ real and correct for this app's architecture, but **not emulator-verified**; run `firebase emulators:exec` before relying on this as the sole proof in an environment that has the emulator installed |
| Observability (Phase 5 alert catalog) | `infrastructure/terraform/monitoring.tf` — extends Phase 2f's 3 baseline alerts with 10 more: Cloud Run p95 latency, Cloud Run Job failure rate, 3 Cloud SQL health alerts, and 4 log-based-metric alerts (stuck-claimed-message rate, required-Gemini-stage fallback rate, report-generation failure rate, agent-framework cold-start fallback rate) keyed on log lines that already exist in the codebase (quoted verbatim in each resource's comment, not invented) | `tests/test_terraform_monitoring.py` (5/5 — offline HCL structural validation via python-hcl2, no `terraform` binary available in this environment) | ⚠️ implemented and structurally validated offline; not yet confirmed against a live `terraform plan` (this environment can't reach the actual project's Metrics Explorer to verify every `metric.type` string resolves) |
| SLO definition | `docs/DURABILITY.md`'s "SLO for the live demo window" section — 10 concrete SLIs with explicit targets, each tied to a specific alert policy above | — | ⚠️ defined as a target; not yet a measured result — no live deployment exists yet to measure against |
| Secret rotation policy | `docs/GOVERNANCE.md`'s "Secret rotation policy" section — versioned-secret rotation procedure built on `tools/secret_resolver.py`'s reference-only contract and 5-minute cache TTL (verified against the actual cache-expiry code, not assumed) | — | ⚠️ policy documented; no live rotation has happened (nothing is deployed live yet) |
| Wave-slot leak detection, assistant quota/safety event rate alerts | Not wired — no existing log line or exported metric to key an alert on | `docs/DURABILITY.md`'s "Known gap" paragraph names the exact instrumentation each would need first | ❌ explicit known gap, not attempted-and-hidden |
| CORS, assistant rate limiting, estate isolation, destructive-write confirmation | Surveyed during Phase 5 and found already solid from earlier work — env-configured CORS allowlist (`frontend/app.py`), real Firestore-transaction assistant quota/concurrency (`frontend/assistant_service.py`), 39 estate-isolation tests (`tests/test_estate_rbac.py`) plus cross-estate assistant/report isolation tests (`tests/test_ai_release_hardening.py`), `Field(min_length=8, max_length=2000)` justification requirement on 9+ destructive-write request models | pre-existing test suites, re-confirmed by reading during this pass, not re-tested from scratch | ✅ already implemented, no Phase 5 changes needed |
